...
function setupEventListeners() {
  ...
  // Manual Match creation submit handler
  document.getElementById('setup-match-manual-form').addEventListener('submit', (e) => {
    e.preventDefault();
    const eventName = document.getElementById('setup-match-event').value;
    const round = document.getElementById('setup-match-round').value;
    const p1Id = document.getElementById('setup-match-p1').value;
    const p2Id = document.getElementById('setup-match-p2').value;
    ...
    const newMatch = {
      ...
      event: eventName,
      round: round,
      roundIndex: 1, // Custom matches default to roundIndex 1
      player1Id: p1Id,
      player2Id: p2Id,
    ...
    saveAndRender();
    alert("已成功新增該場自訂賽事至待排隊列！");
    document.getElementById('setup-match-manual-form').reset();
  });

  // CSV Match batch import handler
  document.getElementById('btn-import-matches').addEventListener('click', () => {
    const importText = document.getElementById('setup-match-import').value;
    const lines = importText.split('\n').map(l => l.trim()).filter(l => l.length > 0);
    
    let successCount = 0;

    lines.forEach(line => {
      // Format: Event, Round, Player1Name, Player2Name (supports commas, tabs, full-width punctuation)
      const parts = line.split(/[,\t，、]/).map(p => p.trim());
      if (parts.length >= 4) {
        const eventName = parts[0];
        const round = parts[1];
        const p1Name = parts[2];
        const p2Name = parts[3];

        if (!eventName || !round || !p1Name || !p2Name) return; // Skip incomplete fields
        if (p1Name === p2Name) return; // skip self-play

        // Check if event exists, if not add it
        if (!state.events.includes(eventName)) {
          state.events.push(eventName);
        }

        // Find or create Player 1
        let p1 = state.players.find(p => p.name === p1Name);
        if (!p1) {
          p1 = {
            id: 'p_' + Math.random().toString(36).substr(2, 9),
            name: p1Name,
            ...
            events: [eventName],
            gift: '無',
            checkedIn: true, // auto check-in imported player for convenience
            giftClaimed: false,
            lastMatchEndedAt: null
          };
          state.players.push(p1);
        } else if (!p1.events.includes(eventName)) {
          p1.events.push(eventName);
        }

        // Find or create Player 2
        let p2 = state.players.find(p => p.name === p2Name);
        if (!p2) {
          p2 = {
            id: 'p_' + Math.random().toString(36).substr(2, 9),
            name: p2Name,
        ...
        } else if (!p2.events.includes(eventName)) {
          p2.events.push(eventName);
        }
    ...
    saveAndRender();
    alert(`成功批次匯入 ${successCount} 場對戰賽事！`);
